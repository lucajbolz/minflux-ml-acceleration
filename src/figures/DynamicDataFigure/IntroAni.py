...
class AnimatedPlot:
    ...
    def set_axis(self, current_time, axes, not_refresh_axes, dx, dy):
        ...
        self.s.drop_axes(axes=[axes['a']])
        self.s.hide_axes(axes=[axes['a']],dirs=['top','right'])
        pass
    ...
    def update(self,frame, axes, filtered_df, names, arrs, points, comPoints, dx, dy, dt, time_values):
        ...
        for idx, axis in zip(range(len(names)),[axes['a']]):
            # select data:
            zi = arrs[frame+idx]
            comx, comy = comPoints[frame+idx]
            x, y = points[frame+idx]
...
def make_animation(plot_style='nature',color_scheme='default',show=True, frames=10,time_bin=50):
    ...
    fig, axs = plt.subplot_mosaic(
        ...
        ,gridspec_kw = {
            "width_ratios": [1]#widths
            ,"height_ratios": [1]#heights
            }
        ,constrained_layout=False
        ,figsize=s.get_figsize(rows=1,cols=1,ratio=1)#
        )
    ...
    names = ['GQ15_MF_scans_modulated_i1_mp2_3']
    ...
    time_values = np.sort(time_values)[::int(unique_times/max_frames)]#np.sort(time_values)[::int(np.ceil(unique_times/max_frames))]
...
if __name__=='__main__':
    ani, fps = make_animation(show=False, frames=80,time_bin=50)
    
    base_dir = os.path.join(ROOT_DIR,'src/figures/DynamicDataFigure/data')
    output_file = base_dir+'/IntroAni.gif'# Define the file name for the GIF
